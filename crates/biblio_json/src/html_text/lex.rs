...
use crate::html_text::parse::{ErrorKind, ParseError};
...
impl Lexer 
{
    ...
    fn parse_tag(&mut self) -> Result<Token, ParseError> {
        ...
        if self.pos >= self.chars.len() {
            return Err(ParseError { kind: ErrorKind::UnexpectedEOF, pos: start });
        }
        ...
        // Parse tag name and attributes - need to handle quoted strings properly
        ...
        // Check if this is an inherently self-closing tag
    ...
    fn parse_attributes(&self, attr_str: &str) -> Result<HashMap<String, String>, ParseError> {
        ...
        while chars.peek().is_some() {
            // Skip whitespace
            while chars.peek() == Some(&' ') || chars.peek() == Some(&'\t') || chars.peek() == Some(&'\n') {
                chars.next();
            }
            ...
            if chars.peek() != Some(&'=') {
                // Attribute without value - use empty string
            ...
            if chars.peek() == Some(&'"') || chars.peek() == Some(&'\'') {
                // Quoted value - consume everything until matching quote
